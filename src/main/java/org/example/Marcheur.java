...
public record Marcheur(String name){
    ...
    private List<Lieu> marcher(Lieu depart, Lieu destination, Set<Rue> ruesDejaVisitees){
        var trajets = new ArrayList<>(List.of(depart));
        ...
        while(!ruePossibles.isEmpty()){
            ...
            var sousTrajet = marcher(prochainDepart, destination, ruesDejaVisitees);
            trajets.addAll(sousTrajet);

            if(sousTrajet.getLast().equals(destination)){
                return trajets;
            }
            trajets.add(depart);
